...
export const taskModule = {
    ...
    switchTaskTab(tab) {
        ...
        document.getElementById('taskStats').style.display = isScheduled ? 'none' : 'grid';
        document.getElementById('taskStatsScheduled').style.display = isScheduled ? 'grid' : 'none';
        document.getElementById('taskToolbar').style.display = isScheduled ? 'none' : 'flex';
        document.getElementById('taskToolbarScheduled').style.display = isScheduled ? 'flex' : 'none';
        document.getElementById('taskList').style.display = isScheduled ? 'none' : 'flex';
        document.getElementById('scheduledTaskList').style.display = isScheduled ? 'flex' : 'none';
    ...
    async showCreateTaskModal() {
        ...
        if (taskTypes.length === 0) {
            taskTypes = [{
                ...
                metadata_schema: {
                    location: {
                        type: 'string', required: true, description: '城市名称',
                        placeholder: '如：北京、上海、深圳'
                    },
                    query_type: {
                        type: 'string', required: false, description: '查询类型',
                        enum: [
        ...
        modal.innerHTML = `
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn-primary" onclick="app.submitCreateTask()">创建普通任务</button>
                </div>
            </div>
        `;
...
${task.started_at ? '开始时间: ' + this.formatDateTime(task.started_at) + '\n' : ''}
${task.completed_at ? '完成时间: ' + this.formatDateTime(task.completed_at) + '\n' : ''}
${task.progress !== undefined ? '进度: ' + task.progress + '%\n' : ''}
${task.retries_attempted > 0 ? '重试次数: ' + task.retries_attempted + '/' + (task.max_retries || 3) + '\n' : ''}
${task.error_message ? '错误信息: ' + task.error_message + '\n' : ''}
${task.metadata ? '元数据: ' + JSON.stringify(task.metadata, null, 2) + '\n' : ''}
                `.trim();
