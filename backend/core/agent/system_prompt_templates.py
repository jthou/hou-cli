...
# ---------------------------------------------------------------------------
# 写文章 / 任务规划（无【核心执行原则】、仅 4 条工具规则）
# 运行时可在首段后注入 planning_context，使用 get_article_writing_system_prompt(planning_context)
# ---------------------------------------------------------------------------
ARTICLE_WRITING_SYSTEM_PROMPT_HEAD = """你是一个智能助手，能够帮助用户解决各种问题。当用户提供历史对话记录时，请基于历史对话内容来理解和回答当前问题。"""
...
# 写文章场景审计说明（仅展示用，运行时在 user 消息中注入）
ARTICLE_WRITING_NOTE = """

【写文章场景说明】user 消息中会注入：当前文章草稿、patch 输出要求（局部修改时只输出 unified diff）、参考 MediaWiki 页面等；若启用任务规划，system 中会追加规划内容。"""
