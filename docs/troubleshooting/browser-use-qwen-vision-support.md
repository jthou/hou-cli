# Browser Tool 支持 Qwen2-VL 视觉功能

## 概述

Browser Tool 现在支持使用 Qwen2-VL 模型来处理需要视觉理解的任务，解决了 DeepSeek 不支持 `use_vision=True` 的问题。
...
QWEN_MODEL=qwen-vl-max  # 可选: qwen-vl-plus, qwen-vl-max

# Browser Tool 配置（可选）
BROWSER_TOOL_USE_VISION=true  # 强制启用视觉功能（默认 false，自动检测）
```
...
Browser Tool 会自动检测任务是否需要视觉功能。如果任务描述中包含以下关键词，会自动使用 Qwen2-VL：
...
**示例：**
```python
# 自动使用 Qwen2-VL（因为包含"截图"关键词）
tool.execute(task="打开网页并截图，告诉我页面内容")

# 自动使用 Qwen2-VL（因为包含"视觉"关键词）
...
设置环境变量 `BROWSER_TOOL_USE_VISION=true` 后，所有任务都会尝试使用 Qwen2-VL（如果 QWEN_API_KEY 已设置）。

## 工作原理

1. **任务分析**: 检查任务描述中是否包含视觉相关关键词
2. **模型选择**: 
   - 如果需要视觉功能且 `QWEN_API_KEY` 已设置 → 使用 Qwen2-VL
   - 否则 → 使用 DeepSeek（默认）
...
1. **API Key**: 需要同时配置 DeepSeek 和 Qwen 的 API Key
2. **成本**: Qwen2-VL 可能比 DeepSeek 更昂贵，需要权衡
3. **兼容性**: Qwen2-VL 兼容 OpenAI API，与 browser-use 兼容
4. **性能**: 视觉模型可能响应更慢，需要合理设置超时时间

## 故障排查

### 问题 1: Qwen API Key 未设置

**错误信息:**
```
WARNING [browser_tool] QWEN_API_KEY 未设置，无法使用视觉功能，回退到 DeepSeek
```

**解决方案:**
- 检查 `.env` 文件中是否设置了 `QWEN_API_KEY`
- 确保 API Key 格式正确

### 问题 2: Qwen API Key 格式无效

**错误信息:**
```
WARNING [browser_tool] QWEN_API_KEY 格式无效，回退到 DeepSeek
```
