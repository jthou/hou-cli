...
#### 步骤 1.2.1: 错误处理（基础版）
- [ ] 添加网络错误处理（超时、连接失败）
- [ ] 添加 API 错误处理（401、429、500等）
- [ ] 实现简单重试机制（固定重试 3 次，间隔 1 秒）
- [ ] 添加错误日志记录
...
#### 步骤 1.2.2: 参数配置（简化版）
- [ ] 支持 `temperature` 参数（默认 0.7）
- [ ] 支持 `max_tokens` 参数（默认 2000）
- [ ] 参数通过环境变量或类初始化传入
...
- [ ] 前端：实时显示流式输出
- [ ] IPC Client：SSE 流式接收
- [ ] API 路由：StreamingResponse 生成
- [ ] Orchestrator：流式处理任务
- [ ] LLM Service：流式 API 调用
