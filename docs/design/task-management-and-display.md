...
### 3.2 列表接口：GET /api/task-queue/tasks
...
### 3.3 详情接口：GET /api/task-queue/tasks/:id

- **用途**：详情弹层/页按需拉取，展示完整结果。
- **返回**：单任务完整行，包含 `result`（JSON 对象）、`metadata` 等全部字段；`result` 即 3.1 约定的结构，前端按 `task_type` 渲染。
